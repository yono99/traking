...
class LaporanController extends Controller
{
    ...
    public function getLaporanUnit(Request $request)
    {
        ...
        try {
            // Data aktivitas harian per status
            ...
            // Total semua aktivitas dalam rentang
            $totalActivities = Activity::whereBetween(
                DB::raw('DATE(created_at)'),
                [$request->start_date, $request->end_date]
            )->count();

            // Distribusi status (untuk donut chart)
            $distribution = Activity::select('status', DB::raw('COUNT(*) as total'))
                ->whereBetween(DB::raw('DATE(created_at)'), [
                    $request->start_date,
                    $request->end_date,
                ])
            ...
            // Volume per unit (berdasarkan mapping status -> unit)
            $unitMap = [
                ...
                'loket_penyerahan' => ['PROSES LOKET PENYERAHAN', 'SELESAI DISERAHKAN'],
            ];

            $volumePerUnit = [];
            foreach ($unitMap as $unit => $statuses) {
                $count = Activity::whereIn('status', $statuses)
                    ->whereBetween(DB::raw('DATE(created_at)'), [
                        $request->start_date,
                        $request->end_date,
                    ])
